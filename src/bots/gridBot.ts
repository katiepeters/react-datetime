...
function handleSinglePair(pair, input) {
	...
	const buyLevels = getPriceLevels(currentPrice, candles, getAmplitude);
...
function getPriceLevels(price, data, getAmplitude) {
	...
	let runInterval = getGridInterval(data, getAmplitude);
...
function getGridInterval(data, getAmplitude) {
...
function checkBuyOrder( order, orderMeta, currentPrice, levelIds ){
	...
	else if( !levelIds.includes(orderMeta.levelId) ){
...
function getBuyValue( trader, currentPrice ){
	trader.getPortfolio
}